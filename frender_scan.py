...
def read_scan_results(csv_file):
    """Reads a frender-scan-results csv file into a list of dicts (same format generated in frender_scan function)"""
    result = []
    with open(csv_file, newline="") as f:
...
def reformat_results(results_list, rc_calls=None):
    """Given a results_list of the format generated by frender_scan or read_scan_results, as well as (optionally) a dict produced by rc_calls, return a dict specifying the sample_name for each barcode"""

    if (rc_mode_test(results_list)) & (rc_calls == None):
        raise SystemExit(
            "The result list appears to have rc data, but no rc_calls dict was provided"
        )
    if (not rc_mode_test(results_list)) & (rc_calls != None):
        raise SystemExit(
            "An rc_calls dict was provided, but the result list does not appear to have rc data"
        )

    results = {}

    if rc_calls:  # we already checked, so can use this as a proxy for rc_mode...
        for entry in results_list:
            if (entry["read_type"] == "demuxable") | (
                entry["rc_read_type"] == "demuxable"
            ):
                f_id, rc_id = entry["sample_name"], entry["rc_sample_name"]
                # in rare cases, a barcode could produce a valid sample_name for forward idx2,
                # and another, different, valid sample_name for rc idx2. Check for this...
                if ("" not in [f_id, rc_id]) & (f_id != rc_id):
                    results[entry["idx1"] + "+" + entry["idx2"]] = "ambiguous"
                else:  # we have only one good, unique sample name
                    sample_name = max(
                        f_id, rc_id
                    )  # pick the longest string, which will be the non-empty one
                    results[entry["idx1"] + "+" + entry["idx2"]] = sample_name

    else:  # more straightforward; if an entry has a sample_name, associate it with that barcode
        for entry in results_list:
            if entry["read_type"] == "demuxable":
                results[entry["idx1"] + "+" + entry["idx2"]] = entry["sample_name"]
            else:
                results[entry["idx1"] + "+" + entry["idx2"]] = entry["read_type"]

    return results


def frender_demux(input_fastqs, results_list, index_hop, ambiguous, rc_mode):
    # For each id in the file, make a call if we will use rc or not (based on sum of reads, prolly)
    rc_calls = call_rc_mode_per_id(results_list)

    # thin down and reformat the results_list to just demuxable barcodes

    base_name = "temp-put-a-function-here"

    r1_files = [
        gzip.open(f"{base_name}_{i}_R1_frender-demux.fq.gz", "wb") for i in rc_calls
    ]
    r2_files = [
        gzip.open(f"{base_name}_{i}_R2_frender-demux.fq.gz", "wb") for i in rc_calls
    ]
    r1_undeter = gzip.open(f"{base_name}_undetermined_R1_frender-demux.fq.gz", "wb")
    r2_undeter = gzip.open(f"{base_name}_undetermined_R2_frender-demux.fq.gz", "wb")
    if index_hop:
        r1_hop = gzip.open(f"{base_name}_index-hop_R1_frender-demux.fq.gz", "wb")
        r2_hop = gzip.open(f"{base_name}_index-hop_R2_frender-demux.fq.gz", "wb")
    if ambiguous:
        r1_ambiguous = gzip.open(f"{base_name}_ambiguous_R1_frender-demux.fq.gz", "wb")
        r2_ambiguous = gzip.open(f"{base_name}_ambiguous_R2_frender-demux.fq.gz", "wb")

    with gzip.open(input_fastqs[0], "rt") as read1, gzip.open(
        input_fastqs[1], "rt"
    ) as read2:
